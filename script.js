...
const CONFIG = {
  ...
  preciosSetup: {
    pequena: 1.0, // < 50g
    mediana: 2.5, // 50g - 250g
    grande: 5.0, // > 250g
  },
  penalizacionColor: 1.5, // Por cada color extra (a partir del segundo)
};
...
function calcularPrecio3D(
  ...
  const subtotalProduccion =
    setupReal + penalizacionMulticolor + costeMaterial + costeMaquina;
  ...
  const cantidadIrpf = baseImponible * CONFIG.irpf;
    ...
    return {
        ...
        costeDiseno: costeDiseno.toFixed(2),
        ...
        margenBruto: (baseImponible - subtotalProduccion - (baseImponible * totalComision) - costeDiseno).toFixed(2)
    };
}
...
const outputs = {
  ...
  desgloseMaquina: document.getElementById("desgloseMaquina"),
    ...
    desgloseDiseno: document.getElementById("desgloseDiseno"),
    rowDiseno: document.getElementById("row-diseno"),
...
function updateCalculations() {
  ...
  const minutosInput =
    parseFloat(document.getElementById("minutos").value) || 0;
  const horas = horasInput + minutosInput / 60;
  ...
  outputs.desgloseMaquina.textContent = `${results.maquina}€`;
    ...
    outputs.desgloseDiseno.textContent = `${results.costeDiseno}€`;
    outputs.rowDiseno.style.display = results.costeDiseno > 0 ? "flex" : "none";
